...
class WCAMenuBase:
    def __init__(self):
            self.loader = WCADataLoader(data_path=Path(__file__).resolve().parent)
            self.data = None
    ...
    def initialize(self):
            ...
            if self.data.get("results", pd.DataFrame()).empty:
                raise FileNotFoundError(
                    "No WCA TSV files were found. Download and extract the TSV export into this folder or a data subfolder."
                )
    ...
    def run(self):
            ...
            try:
                ...
                print(f"\nError: {error}")
                print("Download and extract the WCA TSV export into this project folder or a data subfolder.")
                return
